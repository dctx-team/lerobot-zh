...
在本教程中，我们将展示如何使用异步推理（_async inference_）运行 SmolVLA 的微调版本，以及 LeRobot 支持的所有策略。
**尝试对 LeRobot 支持的所有策略使用异步推理！**
...
简而言之：使用 _异步推理_，你的机器人在策略服务器已经忙于计算下一批动作时继续执行——消除了"等待推理"的延迟，实现更流畅、更具响应性的行为。
这与同步推理（sync）有本质区别，在同步推理中，机器人在策略计算下一批动作时保持空闲。
...
- `POLICY`：要运行的策略类型，以及服务器上要运行的检查点的模型名称/路径。你还需要指定服务器应使用哪个设备，以及一次输出多少动作（上限为策略的最大动作值）。
- `CLIENT`：在向服务器发送新观测之前的块大小阈值，以及在重叠部分聚合动作的函数。可选地，你还可以在运行时可视化队列大小，以帮助你调整 `CLIENT` 参数。

重要的是，

- `actions_per_chunk` 和 `chunk_size_threshold` 是为你的设置调整的关键参数。
- `aggregate_fn_name` 是在重叠部分聚合动作的函数。你可以向函数注册表添加新函数，或在 `robot_client.py` 中添加自己的函数（参见[此处](NOTE:addlinktoLOC)）
...
同步推理依赖于交错的动作块预测和动作执行。这本质上会导致 _空闲帧_，即机器人等待策略输出的帧：一个新的动作块。
反过来，推理受到明显的实时延迟的困扰，机器人由于缺乏可用动作而简单地停止执行。
...
为了克服这一点，我们设计了异步推理，这是一种动作规划和执行解耦的范式，导致 (1) 更高的适应性，最重要的是，(2) 没有空闲帧。
关键的是，使用异步推理，下一个动作块在当前动作块耗尽*之前*计算，导致没有空闲。
通过在重叠部分聚合不同的动作块，确保更高的适应性，获得最新的计划和更紧密的控制循环。
...
<p align="center">
  <i>异步推理</i>不会导致空闲，因为下一个块在当前块耗尽之前计算。
</p>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>0.7</td>
      <td>当队列 ≤ 50% 满时，客户端发送新的观测。 值在 [0, 1] 之间。</td>
    </tr>
  </tbody>
</table>
...
我们发现 `actions_per_chunk` 和 `chunk_size_threshold` 的默认值在我们为 [SmolVLA 论文](https://huggingface.co/papers/2506.01844)开发的实验中效果良好，但建议尝试不同的值以找到最适合你的设置的值。

### 为你的设置调整异步推理

1. **仔细选择你的计算资源。** [PI0](https://huggingface.co/lerobot/pi0) 在推理时占用 14GB 内存，而 [SmolVLA](https://huggingface.co/lerobot/smolvla_base) 仅需要约 2GB。你应该根据你的用例确定最佳计算资源，记住较小的策略需要较少的计算资源。策略和使用的设备（CPU 密集型、使用 MPS 或给定 NVIDIA GPU 上的 CUDA 核心数量）的组合直接影响你应该期望的平均推理延迟。
2. **根据推理延迟调整你的 `fps`。** 当服务器生成新的动作块时，客户端不会空闲，而是在其当前动作队列中步进。如果这两个过程以根本不同的速度发生，客户端可能会最终队列为空。因此，如果你经常耗尽队列中的动作，应该降低你的 fps。
3. **调整 `chunk_size_threshold`**。
   - 接近 `0.0` 的值导致几乎顺序的行为。接近 `1.0` 的值 → 每步发送观测（更多带宽，依赖于良好的世界模型）。
   - 我们发现 0.5-0.6 左右的值效果良好。如果你想调整这个，启动一个 `RobotClient`，将 `--debug_visualize_queue_size` 设置为 `True`。这将在运行时绘制动作队列大小演变，你可以使用它来找到最适合你的设置的 `chunk_size_threshold` 值。
...
如果你想进一步讨论这个问题，请加入我们的 [Discord 社区](https://discord.gg/s3KuuzsPFb)，或在我们的 [GitHub 仓库](https://github.com/huggingface/lerobot/issues)上提出问题。
