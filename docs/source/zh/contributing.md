...
欢迎每个人做出贡献，我们重视每个人的贡献。代码并不是帮助社区的唯一方式。回答问题、帮助他人、联系交流和改进文档都是非常有价值的。
...
- **Issue：** 遵循 [issue 模板](https://github.com/huggingface/lerobot/blob/main/.github/ISSUE_TEMPLATE/bug-report.yml)。
- **Pull request：** 在 `upstream/main` 上进行 rebase，使用描述性分支名称（不要在 `main` 上工作），在本地运行 `pre-commit` 和测试，并遵循 [PR 模板](https://github.com/huggingface/lerobot/blob/main/.github/PULL_REQUEST_TEMPLATE.md)。

> [!IMPORTANT]
> 社区审查政策：为了帮助扩展我们的工作并培养协作环境，我们要求贡献者在自己的 PR 获得关注之前至少审查一个其他人的开放 PR。这种共同责任可以成倍增加我们的审查能力，并帮助每个人的代码更快地合并！
