...
在开始校准和操作之前，您需要识别每个 HopeJR 组件的 USB 端口。运行此脚本以查找手臂、手、手套和外骨骼的 USB 端口：

```bash
lerobot-find-port
```

这将显示可用的 USB 端口及其关联的设备。记下端口路径（例如，`/dev/tty.usbmodem58760433331`、`/dev/tty.usbmodem11301`），因为在记录数据、重放片段或运行遥操作脚本时，您需要在 `--robot.port` 和 `--teleop.port` 参数中指定它们。
...
- **Radial flexor**：将手指底部向拇指移动
- **Ulnar flexor**：将手指底部向小指移动
- **PIP/DIP**：弯曲手指的远端和近端指骨
...
为所有关节设置适当的边界后，单击"Save"将校准值保存到电机。
...
校准完成后，系统将把校准保存到 `/Users/your_username/.cache/huggingface/lerobot/calibration/teleoperators/homunculus_glove/red.json`
...
这将打开一个校准 GUI，您可以在其中设置每个电机的范围限制。手臂运动组织如下：
...
使用校准界面为每个关节设置范围边界。将每个关节移动通过其完整的运动范围，并相应地调整最小值和最大值。为所有关节设置适当的边界后，保存校准。
...
外骨骼允许控制机器人手臂。在校准期间，系统将提示您将所有关节移动通过其完整的运动范围：
...
使用 Hope-JR 进行记录、重放和训练仍处于实验阶段。

### 记录

此步骤记录数据集，可以在[此处](https://huggingface.co/datasets/nepyope/hand_record_test_with_video_data/settings)看到示例。
...
此训练运行可以在[此处](https://wandb.ai/tino/lerobot/runs/rp0k8zvw?nw=nwusertino)查看示例。
