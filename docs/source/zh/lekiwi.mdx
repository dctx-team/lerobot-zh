...
请遵循此 [README](https://github.com/SIGRobotics-UIUC/LeKiwi)。它包含物料清单，以及采购零件的链接，以及 3D 打印零件的说明。
如果这是您第一次打印或者您没有 3D 打印机，请参考建议。

### 有线版本

如果您有**有线** LeKiwi 版本，可以跳过 Raspberry Pi 的安装和 SSH 设置。您还可以直接在 PC 上运行所有命令，包括 LeKiwi 脚本和用于遥操作的主动臂脚本。
...
有关设置 Raspberry Pi 及其 SD 卡的信息，请参阅：[Setup PI](https://www.raspberrypi.com/documentation/computers/getting-started.html)。这里说明了如何下载 [Imager](https://www.raspberrypi.com/software/) 来安装 Raspberry Pi OS 或 Ubuntu。

### 设置 SSH

设置好 Pi 后，您应该启用并设置 [SSH](https://www.raspberrypi.com/news/coding-on-raspberry-pi-remotely-with-visual-studio-code/)（安全外壳协议），以便您可以从笔记本电脑登录到 Pi，而无需在 Pi 上使用屏幕、键盘和鼠标。有关如何执行此操作的精彩教程可以在[此处](https://www.raspberrypi.com/documentation/computers/remote-access.html#ssh)找到。登录到您的 Pi 可以在命令提示符（cmd）中完成，或者如果您使用 VSCode，可以使用[此](https://marketplace.visualstudio.com/items?itemName=ms-vscode-remote.remote-ssh)扩展。

### 在 Pi 上安装 LeRobot 🤗

在您的 Raspberry Pi 上使用我们的[安装指南](./installation)安装 LeRobot
...
配置电机的说明可以在 SO101 [文档](./so101#configure-the-motors)中找到。除了手臂电机的 ID 外，我们还需要设置移动底座的电机 ID。这些需要按特定顺序才能工作。下面是移动底座的电机 ID 和电机安装位置的图像。请注意，我们在 LeKiwi 上只使用一个电机控制板。这意味着轮子的电机 ID 是 7、8 和 9。
...
我们统一了大多数机器人的校准方法，因此，此 SO100 手臂的校准步骤与 Koch 和 SO101 的步骤相同。首先，我们必须将机器人移动到每个关节处于其范围中间的位置，然后按 `Enter`。其次，我们将所有关节移动通过其完整的运动范围。可以在[此处](https://huggingface.co/docs/lerobot/en/so101#calibration-video)找到 SO101 相同过程的参考视频。
...
> [!TIP]
> 如果您使用不同的键盘，可以在 [`LeKiwiClientConfig`](https://github.com/huggingface/lerobot/blob/main/src/lerobot/robots/lekiwi/config_lekiwi.py) 中更改每个命令的键。
...
我们使用 Hugging Face hub 功能来上传您的数据集。如果您以前没有使用过 Hub，请确保您可以使用写访问令牌通过 cli 登录，此令牌可以从 [Hugging Face 设置](https://huggingface.co/settings/tokens)生成。
...
如果您想更深入地了解这个重要主题，可以查看我们撰写的关于什么是好数据集的[博客文章](https://huggingface.co/blog/lerobot-datasets#what-makes-a-good-dataset)。
...
恭喜 🎉，您的机器人已全部设置好，可以自己学习任务了。按照本教程的训练部分开始训练它：[真实世界机器人入门](./il_robots)
