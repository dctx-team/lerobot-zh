...
"""
此脚本将帮助你将已推送到 hub 的 LeRobot 数据集从代码库版本 2.1 转换到 3.0。它将：

- 生成每个episode的统计信息并写入 `episodes_stats.jsonl`
- 检查新统计信息与旧统计信息的一致性
- 删除已弃用的 `stats.json`
- 更新 `info.json` 中的 codebase_version
- 将此新版本推送到 hub 的 'main' 分支并标记为 "v3.0"
...
def concat_data_files(paths_to_cat, new_root, chunk_idx, file_idx, image_keys):
    # TODO(rcadene): 为了节省内存，使用 Dataset.from_parquet(file) 和 concatenate_datasets
...
def convert_videos_of_camera(root: Path, new_root: Path, video_key: str, video_file_size_in_mb: int):
    ...
    for ep_path in tqdm.tqdm(ep_paths, desc=f"convert videos of {video_key}"):
        ...
        ep_metadata = {
            "episode_index": ep_idx,
            f"videos/{video_key}/chunk_index": chunk_idx,  # 保存文件时将更新
            f"videos/{video_key}/file_index": file_idx,  # 保存文件时将更新
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Hugging Face 上的仓库标识符：一个社区或用户名 `/` 数据集名称 "
        "(例如 `lerobot/pusht`, `cadene/aloha_sim_insertion_human`)。",
    )
    parser.add_argument(
        "--branch",
        type=str,
        default=None,
        help="推送数据集的仓库分支。默认为主分支。",
    )
    parser.add_argument(
        "--data-file-size-in-mb",
        type=int,
        default=None,
        help="文件大小（MB）。数据默认为 100，视频默认为 500。",
    )
    parser.add_argument(
