...
if Version(torch.__version__) > Version("2.5.0"):
    # Flex attention 仅在 torch 2.5 及以上版本中可用
...
def flex_attention_forward(
    ...
    def precomputed_mask_factory(precomputed_mask: torch.Tensor) -> _mask_mod_signature:
        def mask_mod(b, h, q_idx, kv_idx):
            # 危险区域：如果 b, h, q_idx, kv_idx 超出形状范围，会发生设备端断言错误。
            return precomputed_mask[b][h][q_idx][kv_idx]

        return mask_mod

    b_mask, h_mask, q_len, kv_len = causal_mask.shape  # 掩码的形状

    block_size = 128
    q_len_rounded = _round_up_to_multiple(q_len, block_size)
    kv_len_rounded = _round_up_to_multiple(kv_len, block_size)

    # *关键* 我们确实需要在这里扩展，否则会出现 CUDA 索引错误
    ...
    #  掩码在内核内部应用，理想情况下比 score_mod 更高效。
    attn_output, attention_weights = flex_attention(
        query_states,
        key_states,
        value_states,
        block_mask=block_mask,
        enable_gqa=True,  # 因为我们为 GQA 调整了 query/key states 的形状
    ...
    attn_output = attn_output.reshape(
        batch_size,
        -1,
        attn_output.shape[2] * attn_output.shape[3],  # 合并 [H, head_dim]
    )
    return attn_output
