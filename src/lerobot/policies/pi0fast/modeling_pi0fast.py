...
"""
π0+FAST: 视觉-语言-动作模型的高效动作分词化

[论文](https://huggingface.co/papers/2501.09747)
[Jax代码](https://github.com/Physical-Intelligence/openpi)

由Physical Intelligence设计。由Hugging Face从Jax移植。
免责声明：预计性能不如原始实现。

微调pi0+FAST预训练模型的示例（`openpi`中的`pi0_fast_base`）：
```bash
lerobot-train \
...
def safe_arcsin(value):
    # 这确保输入保持在[-1,1]范围内，以避免arcsin的无效值
    return torch.arcsin(torch.clamp(value, -1.0, 1.0))


def aloha_gripper_to_angular(value):
    # Aloha将夹爪位置转换为线性空间。以下代码
    # 反转此转换，以与在角度空间中预训练的pi0保持一致。
    #
    # 这些值来自Aloha代码：
    # PUPPET_GRIPPER_POSITION_OPEN, PUPPET_GRIPPER_POSITION_CLOSED
    value = unnormalize(value, min_val=0.01844, max_val=0.05800)

    # 这是Interbotix代码中角度到线性转换的逆过程。
    ...
    # 归一化到[0, 1]。
    # 值0.4和1.5是在实际的Trossen机器人上测量的。
    return normalize(value, min_val=0.4, max_val=1.5)


def aloha_gripper_from_angular(value):
    # 从pi0使用的夹爪位置转换为Aloha使用的夹爪位置。
    # 注意，单位仍然是角度，但范围不同。
...
def aloha_gripper_from_angular_inv(value):
    # 直接反转gripper_from_angular函数。
    value = unnormalize(value, min_val=-0.6213, max_val=1.4910)
    return normalize(value, min_val=0.4, max_val=1.5)


class PI0FASTPolicy(PreTrainedPolicy):
    """围绕PI0FAST分词器和模型的包装类，用于在LeRobot内训练和运行推理。"""
    ...
    def __init__(
        ...
        """
        参数：
            config: 策略配置类实例或None，如果为None，则使用配置类的默认实例化。
            dataset_stats: 用于归一化的数据集统计信息。如果未在此处传递，预期它们将在
                策略使用前通过调用`load_state_dict`传递。
        """
    ...
    def from_pretrained(cls, *args, **kwargs):
        """覆盖from_pretrained方法以显示重要的免责声明。"""
        print(
            "⚠️  免责声明：PI0FAST模型由Hugging Face团队从JAX移植。\n"
            "   预计性能不如原始实现。\n"
            "   原始实现：https://github.com/Physical-Intelligence/openpi"
        )
    ...
    def select_action(self, batch: dict[str, Tensor]) -> Tensor:
        ...
        此方法封装`select_actions`以便一次返回一个动作在环境中执行。
        ...
        # n_action_steps > 1时的动作队列逻辑。当action_queue耗尽时，通过
        # 查询策略来填充它。
        if len(self._action_queue) == 0:
            ...
            # `self.model.forward`返回一个(batch_size, n_action_steps, action_dim)张量，但队列
            # 有效地具有形状(n_action_steps, batch_size, *)，因此进行转置。
...
def block_causal_update_causal_mask(
    ...
    """
    在训练和生成期间更新因果掩码。它可以自定义为不同的注意力掩码。
    """
...
def prepare_inputs_for_generation(
    ...
    **kwargs,
    ...
    # 覆盖 -- 自定义`position_ids`和`pixel_values`处理
    ...
    # Paligemma中的Position_ids是从1开始索引的
    if model_inputs.get("position_ids") is not None:
        model_inputs["position_ids"] += 1
    # 如果我们处于缓存解码阶段，像素值应该为None，因为输入id不再包含特殊图像token
    # 否则我们需要将像素值传递给模型。注意：use_cache=False始终需要pixel_values
...
class PI0FAST(nn.Module):
    def __init__(self, config: PI0FASTConfig):
        ...
        # TODO: 一旦我们将transformers升级到>4.52.0，就删除此行，因为该属性将被删除
    ...
    def set_requires_grad(self):
        ...
        if self.config.freeze_lm_head:
            for name, params in self.pi0_paligemma.named_parameters():
                if "embed_tokens" in name:  # lm头和嵌入层是绑定的
                    params.requires_grad = False
    ...
    def fast_tokenizer_wrapper(self, actions_norm):
        """
        self.fast_tokenizer的包装器，确保批处理、
        转换为PyTorch张量，并返回不带填充的字典。
        """
    ...
    def forward(self, batch: dict[str, Tensor]):
        ...
        # 左移以进行下一步预测
        logits = logits[:, :-1, :]
        targets = targets[:, 1:].to(device)  # 移位目标
        loss_mask = loss_mask[:, 1:].to(device)  # 确保形状正确
        ...
        for token in tokens:
            try:
                ...
                if relaxed_decoding:
                    ...
                    # 如果太长则应用截断
                    if diff < 0:
                        decoded_dct_coeff = decoded_dct_coeff[:expected_seq_len]  # 在右侧截断
                    # 如果太短则应用填充
                    elif diff > 0:
    ...
    def extract_actions(self, tokens: torch.Tensor, action_horizon: int, action_dim: int) -> torch.Tensor:
        """
        使用FAST模型从预测的输出token中提取动作。

        参数：
            tokens (torch.Tensor): 分词化输出的输入张量。
            action_horizon (int): 动作的时间步数。
            action_dim (int): 每个动作的维度。

        返回：
            torch.Tensor: 提取的动作作为形状为(action_horizon, action_dim)的张量。
        """
        # 解码预测的输出token
    ...
    def embed_inputs(
        ...
        # TODO: 避免在python中使用list和torch.cat；更倾向于使用torch.empty预分配
        # 图像是相同大小的列表
        # 向量化所有内容！
        device = images[0].device
        image_embedding_dim = images[0].shape[-1]  # TODO 应该来自self.config
        ...
        b_n, p, image_embedding_dim = embedded.shape  # 提取当前维度
        m = b_n // b  # 动态计算每个样本的图像数量
        ...
        embedded = embedded.reshape(b, n * num_img_emb, image_embedding_dim)  # 形状：(B, N*P, D)
        ...
        # 将填充token移到左侧（.generate()）或右侧（.train()）
