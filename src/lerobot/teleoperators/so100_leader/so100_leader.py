...
class SO100Leader(Teleoperator):
    """
    由 TheRobotStudio 设计的 [SO-100 Leader Arm](https://github.com/TheRobotStudio/SO-ARM100)
    """
    ...
    def calibrate(self) -> None:
        if self.calibration:
            ...
            user_input = input(
                f"按 ENTER 使用与 ID {self.id} 关联的现有校准文件，或输入 'c' 并按 ENTER 运行校准："
            )
        ...
        print(
            f"依次移动除 '{full_turn_motor}' 外的所有关节，使其通过整个运动范围。\n正在记录位置。按 ENTER 停止..."
        )
    ...
    def setup_motors(self) -> None:
        for motor in reversed(self.bus.motors):
            input(f"仅将控制板连接到 '{motor}' 电机并按 Enter。")
